...
async function testLLMIntegration() {
  try {
    ...
    console.log(
      '- OpenAI API Key:',
      process.env.OPENAI_API_KEY ? 'Key exists (hidden for security)' : 'Missing'
    );
    console.log(
      '- Primary Chat Model:',
      process.env.OPENAI_PRIMARY_CHAT_MODEL || 'gpt-4o-mini (default)'
    );
    console.log(
      '- Fallback Chat Model:',
      process.env.OPENAI_FALLBACK_CHAT_MODEL || 'gpt-3.5-turbo (default)'
    );
    console.log(
      '- Embedding Model:',
    ...
    console.log('\n✅ All LLM functionality tests passed!');
  } catch (error) {
