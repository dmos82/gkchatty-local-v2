...
router.post(
  ...
  async (req: Request, res: Response): Promise<Response | void> => {
    ...
    try {
      ...
      // Search by both username and email to handle either input
      ...
      await user.save(); // Save the updated session array to the database
      logger.info(
      ...
      logger.error({ error }, 'Server error during login');
