...
async function startServer() {
  try {
    ...
    app.get('/', (_req: Request, res: Response) => {
      res.status(200).json({
        status: 'ok',
        deployment_version: 'v2.0.1-fix-new-conversation-delivery',
      });
    });
    ...
    // Embeddings Routes (GKChatty Local)
    try {
      console.log('>>> [App Setup] Mounting /api/embeddings routes...');
      app.use('/api/embeddings', embeddingsRoutes);
      console.log('>>> [App Setup] /api/embeddings routes registered.');
    } catch (err) {
      console.error('>>> !!! [App Setup] Error registering /api/embeddings routes:', err);
    }
