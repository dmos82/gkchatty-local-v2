...
export default function DocumentManagerPage() {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (presignedUrl.startsWith('/')) {
            ...
            uploadUrl = `${API_BASE_URL}${presignedUrl}`;
          ...
          const processResponse = await fetch(
            `${API_BASE_URL}/api/documents/process-uploaded-file`,
            {
              method: 'POST',
              credentials: 'include',
              headers: {
  ...
  const downloadDocument = async (doc: UserDocument) => {
    ...
    try {
      ...
      console.log(`[DocDownload] Fetching presigned URL from: ${fetchUrl}`);
  ...
  const handleViewDocument = async (doc: UserDocument) => {
    ...
    try {
      ...
      console.log(`[DocumentsPage] Fetching PDF from URL: ${fetchUrl}`); // Use fetchUrl
      const response = await fetch(fetchUrl, {
        // Use fetchUrl
        method: 'GET',
        credentials: 'include', // CONFIRMED: Already present here
      });
