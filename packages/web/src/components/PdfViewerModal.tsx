...
const PdfViewerModal: React.FC<PdfViewerModalProps> = ({
  ...
  console.log('[PdfViewerModal] Received props:', {
    isOpen,
    fileUrl,
    initialPageNumber,
    chunkText,
  ...
  const customTextRenderer = useCallback(
    (textItem: { str: string }) => {
      ...
      console.log('[Renderer] Comparing:', {
        itemStr: textItem.str,
        normChunk: normalizedChunkText,
      });
      ...
      if (normalizedChunkText.includes(normalizedTextItem) && normalizedTextItem.length > 0) {
        // Add check for non-empty normalized item
        // Log match found
  ...
  const onDocumentLoadError = (error: Error): void => {
    ...
    setLoadError(error); // Store the actual error object
    setLoading(false);
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        ...
        className="p-0 border-0 max-w-4xl max-h-[90vh] overflow-hidden"
      >
        <div className="bg-background rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col text-foreground">
          <div className="flex justify-between items-center p-4 border-b border-border sticky top-0 bg-background z-10">
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
              <a
                ...
                style={{
                  pointerEvents: fileUrl.startsWith('blob:') ? 'auto' : 'none',
                  opacity: fileUrl.startsWith('blob:') ? 1 : 0.5,
                }}
              >
          ...
          <div className="flex-1 overflow-auto p-4 flex flex-col items-center">
            ...
            {loadError && (
              <div className="bg-red-50 text-red-600 rounded-lg p-6 mt-4 max-w-lg mx-auto text-center">
                ...
                <p className="mt-2">
                  {loadError?.message || 'Please try again or use a different document.'}
                </p>
                <button
                  onClick={onClose}
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-64 p-3 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10 text-xs text-gray-700">
                    ...
                    <ul className="space-y-1">
                      <li>
                        <span className="font-medium">← or PageUp:</span> Previous page
                      </li>
                      <li>
                        <span className="font-medium">→ or PageDown:</span> Next page
                      </li>
                      <li>
                        <span className="font-medium">Home:</span> First page
                      </li>
                      <li>
                        <span className="font-medium">End:</span> Last page
                      </li>
                    </ul>
                  </div>
                </div>
              ...
              <Document
                ...
                loading={
                  <div className="flex justify-center items-center p-4">
                    ...
                    <p className="ml-2 text-gray-600">Loading document...</p>
                  </div>
                }
                options={{
                  cMapUrl: '/cmaps/',
