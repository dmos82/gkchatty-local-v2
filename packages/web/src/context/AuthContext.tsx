      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.user) {
            ...
            // DO NOT call checkSession() here anymore.
            console.log(
              `[AuthContext][login][Diag] Login successful. User state set directly. isLoading will be set to false in finally.`
            );
          } else {
            console.error(
        ...
        // Now that checkSession is not called from within login's success path,
        // we need to manage isLoading state directly in login's finally block.
        console.log(
          `[AuthContext][login][Diag] Login function finally block. Setting isLoading to false. Previous isLoading: ${isLoading}`
        );
        setIsLoading(false);
        console.log(
          `[AuthContext][login][Diag] Final state before login function returns: user=${JSON.stringify(user)}, isLoading=${false}, loginAttemptSuccessful=${loginAttemptSuccessful}`
        );
      }
      return loginAttemptSuccessful;
    },
    // Removed checkSession from dependencies as it's no longer called directly in the success path.
    // User and isLoading are used for logging the state at the time of function definition/call.
    // setUser is a stable function from useState.
