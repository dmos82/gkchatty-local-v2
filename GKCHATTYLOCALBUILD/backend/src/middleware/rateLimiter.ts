...
// CRITICAL: Local desktop app mode - disable rate limiting entirely
const isLocalMode = process.env.USE_SQLITE === 'true' || process.env.USE_LOCAL_VECTORS === 'true';
if (isLocalMode) {
  logger.info('🏠 LOCAL MODE DETECTED - Rate limiting DISABLED (local desktop application)');
}
...
export const standardLimiter = rateLimit({
  ...
  skip: () => isLocalMode, // DISABLE rate limiting in local desktop mode
...
export const authLimiter = rateLimit({
  ...
  skip: () => isE2ETesting || isLocalMode, // Disable rate limiting during E2E tests OR local mode
  store: redisStore || undefined,
  handler: (req: Request, res: Response) => {
    logger.warn(
      {
...
export const uploadLimiter = rateLimit({
  ...
  store: redisStore || undefined,
  skip: (req: Request) => {
    // CRITICAL: Skip ALL rate limiting in local desktop mode
