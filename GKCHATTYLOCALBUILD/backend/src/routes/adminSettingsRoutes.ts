...
router.get('/', async (req: Request, res: Response) => {
  logger.info('Admin Settings GET handler - fetching all settings');
  ...
  try {
    ...
    const availableModels = ALLOWED_OPENAI_MODELS;

    return res.status(200).json({
      success: true,
      settings: {
        openai: {
          apiKey: openaiConfig.apiKey ? maskApiKey(openaiConfig.apiKey) : null,
          primaryModel: openaiConfig.primaryModel,
