...
// Predefined list of allowed models for validation (Updated January 2025)
export const ALLOWED_OPENAI_MODELS = [
  // === LOCAL EMBEDDING MODELS (Zero Cost, MPS Optimized) ===
  'local:nomic-embed-text-v1.5', // Primary local model - 768 dimensions, MPS optimized
  'local:all-MiniLM-L6-v2', // Alternative local model - 384 dimensions
  'local:gte-small', // General Text Embeddings - Small
...
// Helper to check if a model is local
export function isLocalModel(modelId: string | null): boolean {
  return modelId?.startsWith('local:') || false;
}
