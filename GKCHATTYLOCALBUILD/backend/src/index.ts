...
import uploadRoutes from './routes/uploadRoutes'; // <-- Import upload routes (local file upload)
...
import modelsRoutes from './routes/modelsRoutes'; // <-- Import models routes
...
async function startServer() {
  try {
    ...
    // Skip MongoDB seeders when using SQLite
    if (process.env.USE_SQLITE !== 'true') {
      ...
      console.log('[Settings Seeder] Skipped (USE_SQLITE=true) - SQLite mode enabled');
    }
    ...
    if (process.env.USE_SQLITE !== 'true') {
      ...
      console.log('[Admin Seeder] Skipped (USE_SQLITE=true) - SQLite mode enabled');
    }
    ...
    if (process.env.USE_SQLITE !== 'true') {
      ...
      console.log('[Persona Seeder] Skipped (USE_SQLITE=true) - SQLite mode enabled');
    }
    // --- END: Default Persona Seeder ---

    // --- BEGIN: SQLite Admin User Seeder ---
    if (process.env.USE_SQLITE === 'true') {
      try {
        console.log('[SQLite Admin Seeder] Initializing SQLite database and creating admin user...');

        // Import SQLite adapter
        const { UserModel, initializeStorage } = await import('./utils/modelFactory');

        // Ensure database is initialized
        initializeStorage();

        const ADMIN_USERNAME = process.env.TEMP_ADMIN_USERNAME || 'admin';
        const ADMIN_PASSWORD = process.env.TEMP_ADMIN_PASSWORD || 'admin';
        const ADMIN_EMAIL = process.env.TEMP_ADMIN_EMAIL || 'admin@gkchatty.local';

        // Check if admin user already exists
        const existingAdmin = UserModel.findOne({ username: ADMIN_USERNAME });

        if (!existingAdmin) {
          console.log(`[SQLite Admin Seeder] Creating admin user '${ADMIN_USERNAME}'...`);
          ...
          console.log(`[SQLite Admin Seeder] ✅ Admin user created successfully!`);
          console.log(`[SQLite Admin Seeder] Username: ${ADMIN_USERNAME}`);
          console.log(`[SQLite Admin Seeder] Password: ${ADMIN_PASSWORD}`);
          console.log(`[SQLite Admin Seeder] Email: ${ADMIN_EMAIL}`);
        } else {
          console.log(`[SQLite Admin Seeder] Admin user '${ADMIN_USERNAME}' already exists.`);
        }
      } catch (error) {
        console.error('[SQLite Admin Seeder] Error creating admin user:', error);
        // Non-fatal - continue startup
      }
    }
    // --- END: SQLite Admin User Seeder ---
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        console.error('Error is not an Error instance:', JSON.stringify(err, null, 2));
      }
    }
    ...
    // Upload Routes (local file upload - replaces S3 presigned URL flow)
    try {
      console.log('>>> [App Setup] Mounting /api/upload routes...');
      app.use('/api/upload', uploadRoutes);
      console.log('>>> [App Setup] /api/upload routes registered.');
    } catch (err) {
      console.error('>>> !!! [App Setup] Error registering /api/upload routes:', err);
    }
    ...
    // --- Models Routes ---
    try {
      console.log('>>> [App Setup] Registering models routes...');
      app.use('/api/models', modelsRoutes);
      console.log('>>> [App Setup] Models routes registered successfully.');
    } catch (err) {
      console.error('>>> !!! [App Setup] Error registering models routes:', err);
    }
