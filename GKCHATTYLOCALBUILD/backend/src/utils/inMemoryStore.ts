...
const log = getLogger('inMemoryStore');
...
function cosineSimilarity(vecA: number[], vecB: number[]): number {
  if (!vecA || !vecB || vecA.length !== vecB.length) {
    log.error('Cosine similarity error: Invalid or mismatched vectors.', {
      lenA: vecA?.length,
      lenB: vecB?.length,
    });
    return 0;
  ...
  for (let i = 0; i < vecA.length; i++) {
    ...
    if (typeof valA !== 'number' || typeof valB !== 'number' || isNaN(valA) || isNaN(valB)) {
      log.error('Invalid value in vector during cosine similarity.', { index: i, valA, valB });
      return 0;
    }
...
export function addDocumentChunk(chunk: InMemoryDocumentChunk): void {
  ...
  if (!chunk?.metadata?.chunkId || !chunk.pageContent || !chunk.embedding?.length) {
    log.error('[InMemoryStore] Attempted to add invalid chunk:', chunk);
    return;
  }
  if (chunk.embedding.some(isNaN)) {
    log.error(`[InMemoryStore] Chunk ${chunk.metadata.chunkId} embedding contains NaN values.`);
    return;
  }
  ...
  if (exists) {
    log.warn(`[InMemoryStore] Chunk ${chunk.metadata.chunkId} already exists. Skipping.`);
    return;
  }

  documentStore.push(chunk);
  // log.debug(`[InMemoryStore] Added chunk ${chunk.metadata.chunkId}. Total chunks: ${documentStore.length}`); // Keep less verbose
}
...
export function similaritySearch(queryEmbedding: number[], topK: number): InMemoryDocumentChunk[] {
  log.debug(
    `[InMemoryStore] similaritySearch started. Comparing against ${documentStore.length} chunks.`
  );
  if (documentStore.length === 0 || topK <= 0) {
    return [];
  }
  if (!queryEmbedding || queryEmbedding.length === 0 || queryEmbedding.some(isNaN)) {
    log.error('[InMemoryStore] Invalid query embedding for similarity search.');
    return [];
  }
  ...
  const scoredChunks = documentStore
    .map(chunk => {
      if (
        ...
        chunk.embedding.some(isNaN)
      ) {
        log.warn(
          `[InMemoryStore] Skipping chunk ${chunk.metadata.chunkId} due to invalid/mismatched embedding.`
        );
  ...
  log.debug(`[InMemoryStore] similaritySearch finished. Returning ${topResults.length} results.`);
  return topResults;
}
...
export function clearStore(): void {
  ...
  log.debug('[InMemoryStore] Store cleared.');
}

// Log initialization
log.debug('[InMemoryStore] Utility initialized.');
