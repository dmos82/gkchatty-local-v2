...
import { UserDocumentModel as UserDocument, IUserDocument } from '../utils/modelFactory'; // Import UserDocument model and interface
import { SystemKbDocumentModel as SystemKbDocument, ISystemKbDocument } from '../utils/modelFactory'; // ADDED: Import SystemKbDocument and interface
...
import { generateEmbeddings } from './transformersHelper';
// Use local LanceDB instead of Pinecone
import { upsertVectors } from './lancedbService';
...
/**
 * Processes a document: fetches from S3, extracts text, splits, generates embeddings using Transformers.js,
 * and upserts to ChromaDB (local vector storage).
 * Handles 'user', 'system', and 'tenant' source types.
  ...
  try {
    ...
    if (isPdf) {
      ...
      try {
        ...
        // If no text extracted, PDF may be image-based (scanned)
        if (!fullText || fullText.length === 0) {
          log.warn(
            `[DocProcessor - ${correlationId}] No text extracted from PDF. This PDF appears to be image-based (scanned). ` +
            `Image-based PDFs require OCR (Optical Character Recognition) to extract text. ` +
            `Please convert the PDF to a text-based PDF or provide the document in a different format (TXT, MD, DOCX).`
          );
          // Provide helpful error message to user
          errorCode = IngestionErrorCode.NO_TEXT_EXTRACTED;
          throw new Error(
            `This PDF is image-based (scanned) and cannot be processed. ` +
            `Please upload a text-based PDF or convert it using an OCR tool first.`
          );
        }
      } catch (pdfError: any) {
        // If it's our intentional NO_TEXT_EXTRACTED error, rethrow it
        if (pdfError.message.includes('image-based')) {
          throw pdfError;
        }
    ...
    try {
      ...
      throw new Error(`Transformers.js embedding generation failed: ${embedError.message}`);
    }
    ...
    log.info(`[DocProcessor - ${correlationId}] Starting ChromaDB upsert phase`);
    await updateDocStatus({
      status: 'processing',
    ...
    log.info(
      `[DocProcessor - ${correlationId}] Preparing ${textChunks.length} vectors for ChromaDB upsert`
    );

    // Prepare vectors in ChromaDB format
    ...
    textChunks.forEach((chunkText, index) => {
      ...
      vectorIds.push(`${documentId}_chunk_${index}`);
      vectorValues.push(embeddings[index]);
      vectorMetadata.push(finalMetadata);
    });
    ...
    log.info(
      `[DocProcessor - ${correlationId}] Upserting ${vectorIds.length} vectors to ChromaDB collection: "${targetNamespace}"`
    );
    await updateDocStatus({
      status: 'processing',
      statusDetail: `Embedding complete. Upserting to ChromaDB collection: ${targetNamespace}.`,
      totalChunks: textChunks.length,
    });

    // Upsert vectors to ChromaDB
    await upsertVectors(targetNamespace, {
      vectors: vectorValues,
      ids: vectorIds,
      metadata: vectorMetadata,
    });

    log.info(`[DocProcessor - ${correlationId}] ChromaDB upsert successful`);
