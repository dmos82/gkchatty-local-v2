 ...
 */
export const getFile = async (s3Key: string): Promise<Buffer> => {
  const filePath = path.join(baseDir, s3Key);

  try {
    const buffer = await readFile(filePath);
    log.debug(`[LocalStorageHelper] File read successfully: ${filePath}`);
    return buffer;
  } catch (error: unknown) {
    log.error(`[LocalStorageHelper] Error reading file ${filePath}:`, error);
    throw error;
  }
};
 ...
 */
export const getFileStream = (s3Key: string): NodeJS.ReadableStream => {
  const filePath = path.join(baseDir, s3Key);
  log.debug(`[LocalStorageHelper] Creating file stream: ${filePath}`);
