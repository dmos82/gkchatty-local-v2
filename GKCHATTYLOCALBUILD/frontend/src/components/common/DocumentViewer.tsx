...
export function DocumentViewer({ documentId, filename, type, onClose }: DocumentViewerProps) {
  ...
  useEffect(() => {
    ...
    const fetchDocument = async () => {
      ...
      let targetUrl;
      if (type === 'system') {
        targetUrl = `${API_BASE_URL}/api/system-kb/download/${documentId}`;
      } else if (type === 'user') {
        targetUrl = `${API_BASE_URL}/api/documents/view/${documentId}`;
      } else {
        if (isMounted) {
      ...
      try {
        console.log('[DocumentViewer] Fetching document metadata from:', targetUrl);
