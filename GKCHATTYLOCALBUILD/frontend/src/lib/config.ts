...
if (process.env.NODE_ENV === 'production') {
  ...
  if (!resolvedClientUrl) {
    console.warn(
      'WARNING: NEXT_PUBLIC_API_BASE_URL is not set. Falling back to http://localhost:6001 for local client development.'
    );
    resolvedClientUrl = 'http://localhost:6001';
  }
...
if (typeof window === 'undefined') {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    resolvedInternalUrl = API_INTERNAL_URL_FROM_ENV || 'http://localhost:6001';
    if (!API_INTERNAL_URL_FROM_ENV) {
      console.warn(
        'WARNING: API_INTERNAL_URL is not set. Falling back to http://localhost:6001 for local server development.'
      );
    }
  }
} else {
