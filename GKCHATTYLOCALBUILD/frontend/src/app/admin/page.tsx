...
export default function AdminPage() {
  ...
  const fetchSystemGrandTotals = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/stats/summary${queryString}`, {
        method: 'GET',
        credentials: 'include',
      });
      if (!response.ok) {
  ...
  const fetchSystemPrompt = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/settings/system-prompt`, {
        credentials: 'include',
      });
      if (!response.ok) {
        const errorData = await response
  ...
  const handleSetRole = async (userId: string, newRole: 'admin' | 'user') => {
    ...
    const apiUrl = `${API_BASE_URL}/api/admin/users/${userId}/role`;
  ...
  const handleUpload = async (filesToUpload?: FileList | null) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const apiUrl = `${API_BASE_URL}/api/admin/system-kb/upload`;
      try {
        const response = await fetch(apiUrl, {
          method: 'POST',
          credentials: 'include',
  ...
  const handleViewSystemKbDocument = async (docId: string, filename: string) => {
    ...
    const fetchUrl = `${API_BASE_URL}/api/admin/system-kb/download/${docId}`;
    // --- END CHANGE ---
