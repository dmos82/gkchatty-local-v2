...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const backendStatsUrl = `${API_BASE_URL}/admin/stats`; // REMOVED /api // Assuming backend has this route
    ...
    if (!response.ok) {
      console.error(
        `[Web API - Admin Stats] Backend API returned error ${response.status}: ${await response.text()}`
      );
      // Forward the backend error status and message if possible
      const errorBody = await response
        .json()
        .catch(() => ({ error: 'Backend API request failed' }));
      return NextResponse.json(
        {
          error:
            errorBody.message || errorBody.error || 'Failed to fetch admin stats from backend.',
        },
