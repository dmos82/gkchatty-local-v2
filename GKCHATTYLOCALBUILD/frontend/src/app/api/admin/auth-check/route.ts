...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const backendCheckUrl = `${API_BASE_URL}/auth/check-session`; // REMOVED /api // Adjust endpoint if needed
    ...
    if (!response.ok) {
      ...
      const errorBody = await response
        .json()
        .catch(() => ({ error: 'Backend session check failed' }));
      console.error(
        `[Web API - Admin Auth Check] Backend API returned error ${errorStatus}:`,
        errorBody
      );
    ...
    console.log(
      '[Web API - Admin Auth Check] Backend session check successful. User data:',
      backendUserData
    );

    if (backendUserData?.user?.role !== 'admin') {
      console.log(
        '[Web API - Admin Auth Check] Backend confirmed user is authenticated but not admin.'
      );
