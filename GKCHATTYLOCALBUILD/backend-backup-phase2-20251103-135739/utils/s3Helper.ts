...
export const getPresignedUrlForView = async (
  ...
  if (IS_LOCAL_STORAGE) {
    ...
    const localFileServePath = `/api/files/local/${encodeURIComponent(finalKey)}`;
    const apiPort = process.env.PORT || '6001';
...
export const getPresignedUrlForPut = async (
  ...
  if (IS_LOCAL_STORAGE) {
    ...
    const localFileUploadPath = `/api/files/local/upload/${encodeURIComponent(finalKey)}`;
    const apiPort = process.env.PORT || '6001';
...
export const saveFile = async (
  ...
  if (IS_LOCAL_STORAGE) {
    // For local storage, return the API path generated by getPresignedUrlForView
    // Note: getPresignedUrlForView will apply the staging prefix internally
    return getPresignedUrlForView(key);
  } else {
    // For S3, return the S3 URL format
    // Important: the key was already prefixed in uploadFile, so use the original key here
    return `https://${BUCKET_NAME}.s3.${S3_REGION}.amazonaws.com/${encodeURIComponent(addStagingPrefix(key))}`;
  }
};
